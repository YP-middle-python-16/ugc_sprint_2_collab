...
def test_send_event_message_success(send_message_mock: MagicMock, client):
    response = client.post(
        "/api/v1/event/", headers={"Content-Type": "application/json"}, json={"key": "test_key", "value": "test_value"}
    )
...
def test_send_event_message_empty_msg_fail(send_message_mock: MagicMock, client):
    response = client.post("/api/v1/event/", headers={"Content-Type": "application/json"})
