...
class ClickhouseStorage(HiLoadStorage):
    ...
    def insert_batch(self, data=None, batch_size: int = 10):
        ...
        s = ","
        batch = s.join(rows)
        query = f"INSERT INTO movies_statistics.view_stat (movie_id, user_id, eventTime, view_run_time) " f"VALUES {batch}"
        self.client.execute(query)

    def select(self, data=None):
        query = f"SELECT  * FROM movies_statistics.view_stat " f"WHERE movie_id = '{data.movie_id}'"
        self.client.execute(query)
